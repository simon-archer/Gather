...
export default function InputField({ setFinalResponseText, isLoading, setIsLoading, voiceId, setVoiceId }) {
  ...
  return h(
    ...
    : h("form", { onSubmit: handleFormSubmit, class: tw`flex flex-col items-center w-full` }, [
        ...
        h("button", { onClick: (event) => handleButtonClick(event, "9dSY1SPd1tIQimc1vGkV"), class: voiceId === "9dSY1SPd1tIQimc1vGkV" ? tw`bg-blue-300 hover:bg-blue-300 rounded-full p-2 text-black text-sm font-semibold mx-2 my-1 focus:outline-none` : tw`bg-gray-200 hover:bg-gray-400 rounded-full p-2 text-black text-sm mx-2 my-1 focus:outline-none`}, "Bosco"),
        h("button", { onClick: (event) => handleButtonClick(event, "CRgVuL7NHLOGdC7AAxb2"), class: voiceId === "CRgVuL7NHLOGdC7AAxb2" ? tw`bg-blue-300 hover:bg-blue-300 rounded-full p-2 text-black text-sm font-semibold mx-2 my-1 focus:outline-none` : tw`bg-gray-200 hover:bg-gray-400 rounded-full p-2 text-black text-sm mx-2 my-1 focus:outline-none`}, "Cynthia"),
        h("button", { onClick: (event) => handleButtonClick(event, "VTvSgMwVP8qbNLRgV9vE"), class: voiceId === "VTvSgMwVP8qbNLRgV9vE" ? tw`bg-blue-300 hover:bg-blue-300 rounded-full p-2 text-black text-sm font-semibold mx-2 my-1 focus:outline-none` : tw`bg-gray-200 hover:bg-gray-400 rounded-full p-2 text-black text-sm mx-2 my-1 focus:outline-none`}, "Alice"),
        h("button", { onClick: (event) => handleButtonClick(event, "rU18Fk3uSDhmg5Xh41o4"), class: voiceId === "rU18Fk3uSDhmg5Xh41o4" ? tw`bg-blue-300 hover:bg-blue-300 rounded-full p-2 text-black text-sm font-semibold mx-2 my-1 focus:outline-none` : tw`bg-gray-200 hover:bg-gray-400 rounded-full p-2 text-black text-sm mx-2 my-1 focus:outline-none`}, "Ryan")
      ]),     
        h("button", { 
          type: "submit", 
