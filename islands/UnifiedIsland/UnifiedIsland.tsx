...
export default function UnifiedIsland() {
  ...
  const [voiceId, setVoiceId] = useState("TJkuZk8hpsh2FDl52cJI");
  ...
  const handleTextGenerated = (text, id) => {
    setFinalResponseText({
      choices: [{ message: { function_call: { arguments: text } } }],
    });
    setTextId(id);
  };
  ...
  return h("div", {
    class:
      tw`flex flex-row items-center justify-center max-h-screen min-h-screen`,
  }, [
    isCollapsed && h(Fragment, {}, [
      finalResponseText && h("button", {
        ...
        class:
          tw`fixed z-10 top-5 right-0 transform -translate-x-1/2 w-12 h-12 bg-[#38A1FF] hover:bg-[#318BDC] text-white font-semibold flex justify-center items-center rounded-full shadow-lg`,
      }, [
        h("svg", {
          class: tw`fill-current text-white`,
          width: "24",
          height: "24",
        }, h("path", { d: "M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" })),
      ]),
      clickedNew && !isLoading &&
      (!selectedItem && (!finalResponseText || finalResponseText === "")) &&
      h("a", {
        href: "https://forms.gle/453jz6DsTpaQMs6Q6",
        class:
          tw`fixed z-10 top-0 left-1/2 transform -translate-x-1/2 mt-4 bg-orange-400 hover:bg-orange-500 text-white font-semibold py-2 px-4 rounded-full`,
        target: "_blank",
      ...
      h("div", {
        ...
        !selectedItem && (!finalResponseText || finalResponseText === "") &&
        h(InputField, {
          setFinalResponseText: handleTextGenerated,
          isLoading,
          setIsLoading,
          voiceId,
          setVoiceId,
